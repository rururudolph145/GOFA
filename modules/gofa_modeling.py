...
from .gnn import GOFAGNNConv, GOFAGNNConvFullAtt, GOFAGNNConvCMB, GOFAGNNConvMLP
from .mplm import MPLMDecoderLayer
...
class MPLMMistralModel(MistralModel):
    ...
    def __init__(self, config: MistralConfig, gofa_config):
        ...
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList(
            [MistralDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers-gofa_config.num_layers)] + [MPLMDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers-gofa_config.num_layers, config.num_hidden_layers)]
        )
        self._use_sdpa = config._attn_implementation == "sdpa"
        self._use_flash_attention_2 = config._attn_implementation == "flash_attention_2"
        self.norm = MistralRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        if use_cache:
            extract_past_key_values_length = past_key_values.get_usable_length(seq_length, layer_idx=len(self.layers)-1)
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if i + self.trainable_layer < len(self.layers):
                with torch.no_grad():
                    layer_outputs = self.forward_layer(i, map_node, hidden_states, cur_node_size, graph, past_key_state, extracts, decoder_layer, past_key_values, output_attentions, use_cache, attention_mask, position_ids)
            else:
                layer_outputs = self.forward_layer(i, map_node, hidden_states, cur_node_size, graph, past_key_state,
